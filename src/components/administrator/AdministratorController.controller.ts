...
export default class AdministratorController extends BaseController {
    ...
    async delete(req: Request, res: Response) {
        ...
        this.services.administrator.getById(administratorId, DefaultAdministratorAdapterOptions)
        .then(result => {
            if (result === null) {
                throw {
                    status: 404,
                    message: 'Administrator nije pronadjen!',
                }
            }
        })
        .then(() => {
            return this.services.administrator.deleteById(administratorId)
        })
        .then(() => {
            res.send('Administrator je izbrisan!');
        })
        .catch(error => {
